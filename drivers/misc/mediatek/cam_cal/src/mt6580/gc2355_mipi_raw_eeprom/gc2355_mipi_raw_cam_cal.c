...
#include "gc2355_mipi_raw_cam_cal.h"
...
/********************************************************************************/
#define GC2355_CAM_CAL_DRVNAME "CAM_CAL_GC2355"
static struct i2c_board_info __initdata kd_cam_cal_dev={ I2C_BOARD_INFO(GC2355_CAM_CAL_DRVNAME, 0xA4>>1)}; //make dummy_eeprom co-exist
...
static dev_t g_CAM_CALdevno = MKDEV(GC2355_CAM_CAL_DEV_MAJOR_NUMBER,0);
...
inline static int RegisterCAM_CALCharDrv(void)
{
    ...
    if( alloc_chrdev_region(&g_CAM_CALdevno, 0, 1,GC2355_CAM_CAL_DRVNAME) )
    {
...
#else
    if( register_chrdev_region(  g_CAM_CALdevno , 1 , GC2355_CAM_CAL_DRVNAME) )
    {
    ...
    CAM_CAL_class = class_create(THIS_MODULE, "GC2355_CAM_CALdrv");
    if (IS_ERR(CAM_CAL_class)) {
        int ret = PTR_ERR(CAM_CAL_class);
        CAM_CALDB("Unable to create class, err = %d\n", ret);
        return ret;
    }
    CAM_CAL_device = device_create(CAM_CAL_class, NULL, g_CAM_CALdevno, NULL, GC2355_CAM_CAL_DRVNAME);
...
static const struct i2c_device_id CAM_CAL_i2c_id[] = {{GC2355_CAM_CAL_DRVNAME,0},{}};   
...
//   .detect = CAM_CAL_i2c_detect,                           
    .driver.name = GC2355_CAM_CAL_DRVNAME,
    .id_table = CAM_CAL_i2c_id,                             
};

#ifndef CAM_CAL_ICS_REVISION
static int CAM_CAL_i2c_detect(struct i2c_client *client, int kind, struct i2c_board_info *info) {         
    strcpy(info->type, GC2355_CAM_CAL_DRVNAME);
    return 0;
}
#endif
static int CAM_CAL_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id) {             
...
//    spin_lock_init(&g_CAM_CALLock);
    ...
    g_pstI2Cclient->addr = GC2355_DEVICE_ID>>1;
    spin_unlock(&g_CAM_CALLock); // for SMP    
